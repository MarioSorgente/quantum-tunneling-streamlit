...
# ---------------------
# Set a Clean, Minimal Style (Apple-like)
# ---------------------
plt.rcParams.update({
...
class QuantumEnsemble:
    ...
    def create_transmission_plot(self, E, V0, L):
        ...
        ax.set_title(f"E = {E:.2f} eV  |  V₀ = {V0:.2f} eV  |  L = {L:.2f} nm", fontsize=16)
    ...
    def create_animation(self, E, V0, L):
        ...
        # Add a clean label for the barrier length at the center of the barrier
        ax.text(L/2, 0.85, f"L = {L:.2f} nm", ha="center", va="bottom", fontsize=12, color="black")

        # Calculate transmission probability T
        T_val = self.transmission(E, V0, L)
        # Fresh randomness each update
        np.random.seed()
        ...
        electrons = [ax.plot([], [], 'o', markersize=12, color=color, alpha=0.9)[0]
                     for color in colors]
...
def main():
    ...
    st.markdown(
        """
        This simulation demonstrates quantum tunneling through a rectangular barrier.
        Adjust the sidebar sliders to change the electron energy, barrier height, and barrier width.
        The top panel shows the theoretical transmission probability vs. energy.
        The bottom panel animates a small ensemble of electrons:
        - **Green** indicates electrons that are transmitted.
        - **Red** indicates electrons that are reflected.
        """
    )
    ...
    # Display the transmission probability plot (top panel)
    fig = ensemble.create_transmission_plot(E, V0, L)
    st.pyplot(fig)

    # Display the animation (bottom panel)
