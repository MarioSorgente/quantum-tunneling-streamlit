...
def build_3d_animation(trajectories, total_frames, V0, L, n_electrons=5):
    ...
    Each electron displays a fading trail (as a low-opacity line) and a fully opaque current marker.
    ...
    for traj in trajectories:
        # Trail trace (starts with the first point)
        data.append(
            go.Scatter3d(
                x=[traj["x"][0]],
                y=[traj["y"][0]],
                z=[traj["z"][0]],
                mode="lines",
                line=dict(color="blue", width=4, opacity=0.3),
                showlegend=False,
            )
        )
        ...
        data.append(
            go.Scatter3d(
                ...
                z=[traj["z"][0]],
                mode="markers",
                marker=dict(color="blue", size=6, opacity=1),
                showlegend=False,
            )
        )
